...
int get_byte(uint32_t vaddr, uint8_t *byte) {
	...
	if (!((pte & VALID_BIT) >> VALID_POS)) {
		PAGE_FAULT(vaddr, "invalid page in get");
		return -1;
	}
	uint8_t bitsToCheck = (uint8_t) ((pte & RWX_BITS) >> RWX_POS);
	...
	case X_PATTERN:
		break;
	default:
		PAGE_FAULT(vaddr, "the page can't be read")
		;
		return -1;
	}
#endif

	page_n = (uint8_t) (pte >> SHIFT_RIGHT);
	offs = (uint16_t) (vaddr & MASK_OFFSET);
...
int store_byte(uint32_t vaddr, uint8_t byte) {
	...
	/* Check pte attributes*/
	if (!((pte & VALID_BIT) >> VALID_POS)) {
		PAGE_FAULT(vaddr, "invalid page in store");
		return -1;
	}
	uint8_t bitsToCheck = (uint8_t) ((pte & RWX_BITS) >> RWX_POS);
	...
	case W_PATTERN:
		break;
	default:
		PAGE_FAULT(vaddr, "the page can't be written")
		;
		return -1;
	}
#endif
